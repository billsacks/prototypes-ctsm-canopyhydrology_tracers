! This version breaks out separate subroutines for anything that operates on tracers.
! This includes summary fluxes (for tracers & bulk), state updates (for tracers & bulk)
! and tracer calculations. It keeps the bulk-only flux calculations inline.
!
! Here (in contrast with the allBrokenOut version), I have put the loop over tracers
! inside the routines to avoid cluttering the science-focused top-level routine.
...
subroutine CanopyHydrology
  ...
  call PrecipInputs(num_soilp, filter_soilp, water_inst)
  ...
  call AddInterceptionToCanopy(num_soilp, filter_soilp, water_inst)
  ...
  call RemoveCanfallFromCanopy(num_soilp, filter_soilp, water_inst)
  ...
  call RemoveSnowUnloading(num_soilp, filter_soilp, water_inst)

  call FluxesOntoGround(num_soilp, filter_soilp, water_inst)

end subroutine CanopyHydrology

subroutine PrecipInputs(num_soilp, filter_soilp, water_inst)
  ! Compute patch-level precipitation inputs for bulk water and all tracers
...
subroutine AddInterceptionToCanopy(num_soilp, filter_soil, water_inst)
...
subroutine RemoveCanfallFromCanopy(num_soilp, filter_soilp, water_inst)
...
subroutine RemoveSnowUnloading(num_soilp, filter_soilp, water_inst)
...
subroutine FluxesOntoGround(num_soilp, filter_soilp, water_inst)
